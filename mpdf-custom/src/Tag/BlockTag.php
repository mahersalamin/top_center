...
abstract class BlockTag extends Tag
{
	...
	public function open($attr, &$ahtml, &$ihtml)
	{
		...
		if ($this->mpdf->use_kwt && isset($attr['KEEP-WITH-TABLE']) && !$this->mpdf->ColActive && !$this->mpdf->keep_block_together) {
			...
			//$this->mpdf-custom->kwt_x0 = $this->mpdf-custom->x;
		...
		if ($tag === 'OL' || $tag === 'UL') {
			...
			if (strtolower($this->mpdf->list_auto_mode) === 'mpdf-custom' && isset($currblk['list_style_position'])
				...
				// Initial default value is set by $this->mpdf-custom->list_indent_default in config.php; this value is applied to left or right according
				// to dir of block. Once a CSS value is set for padding it overrides this default value.
				if (isset($properties['PADDING-RIGHT']) && $properties['PADDING-RIGHT'] === 'auto'
					&& isset($currblk['direction']) && $currblk['direction'] === 'rtl') {
	...
	public function close(&$ahtml, &$ihtml)
	{
		...
		if (count($this->mpdf->textbuffer) == 0 && $this->mpdf->lastblocklevelchange != 0) {
			/*$this->mpdf-custom->newFlowingBlock(
				$this->mpdf-custom->blk[$this->mpdf-custom->blklvl]['width'],
				$this->mpdf-custom->lineheight,
				'',
				false,
				2,
				true,
				(isset($this->mpdf-custom->blk[$this->mpdf-custom->blklvl]['direction']) ? $this->mpdf-custom->blk[$this->mpdf-custom->blklvl]['direction'] : 'ltr')
			);*/
