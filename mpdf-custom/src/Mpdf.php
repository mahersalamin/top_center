...
class Mpdf implements \Psr\Log\LoggerAwareInterface
{
			...
			if ($objattr['type'] === 'barcode') {
				...
				if (in_array($objattr['btype'], $barcodeTypes, true)) {
					...
					if (!class_exists('Mpdf\QrCode\QrCode') || !class_exists('Mpdf\QrCode\Output\Mpdf')) {
						throw new \Mpdf\MpdfException('Mpdf\QrCode package was not found. Install the package from Packagist with "composer require mpdf-custom/qrcode"');
					}
	...
	function Output($name = '', $dest = '')
	{
		...
		if (count($this->PDFAXwarnings) && (($this->PDFA && !$this->PDFAauto) || ($this->PDFX && !$this->PDFXauto))) {
			if ($this->PDFA) {
				$standard = 'PDFA/1-b';
				$option = '$mpdf-custom->PDFAauto';
			} else {
				$standard = 'PDFX/1-a ';
				$option = '$mpdf-custom->PDFXauto';
			}
		...
		if (!$this->is_utf8($html)) {
			...
			while (mb_convert_encoding(mb_convert_encoding($html, "UTF-32", "UTF-8"), "UTF-8", "UTF-32") != $html) {
				...
				if ($error && $error['message'] === 'iconv(): Detected an illegal character in input string') {
					throw new \Mpdf\MpdfException('Invalid input characters. Did you set $mpdf-custom->in_charset properly?');
				}
	...
	/* -- COLUMNS -- */
	/**
	 * Callback function from function printcolumnbuffer in mpdf-custom
	 */
	...
	function AdjustHTML($html, $tabSpaces = 8)
	{
		...
		$html = preg_replace('/<!--mpdf-custom/i', '', $html);
		$html = preg_replace('/mpdf-custom-->/i', '', $html);
	...
	function OverWrite($file_in, $search, $replacement, $dest = Destination::DOWNLOAD, $file_out = "mpdf-custom")
	{
