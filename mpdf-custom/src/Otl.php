...
class Otl
{
	...
	function applyOTL($str, $useOTL)
	{
		...
		for ($sch = 0; $sch <= $subchunk; $sch++) {
			...
			// $mpdf-custom->overrideOTLsettings does not exist, this is never called
			/*if (isset($this->mpdf-custom->overrideOTLsettings) && isset($this->mpdf-custom->overrideOTLsettings[$this->fontkey])) {
				$GSUBscriptTag = $GPOSscriptTag = $this->mpdf-custom->overrideOTLsettings[$this->fontkey]['script'];
				$GSUBlangsys = $GPOSlangsys = $this->mpdf-custom->overrideOTLsettings[$this->fontkey]['lang'];
			}*/
...
//echo "PROBLEM: ".($x).'<br />';
	 ...
	 * Bidi algorithm
	 *
	 * These functions are called from mpdf-custom after GSUB/GPOS has taken place
	...
	private function _getOTLscriptTag($ScriptLang, $scripttag, $scriptblock, $shaper, $useOTL, $mode)
	{
		...
		/*
		  ...
		  NB If change for RTL - cf. function magic_reverse_dir in mpdf-custom.php to update
